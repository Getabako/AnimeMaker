...
export function runJobStream(opts: RunJobOptions): Response {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const model = process.env.ANIMEMAKER_MODEL || "gpt-5.5";
        const effort = process.env.ANIMEMAKER_EFFORT || "medium";
...
/** multipart/form-data から参照画像を取り出して cwd/reference.png に保存。 */
export async function saveReferenceImage(file: File, cwd: string): Promise<void> {
  const buf = Buffer.from(await file.arrayBuffer());
  fs.writeFileSync(path.join(cwd, "reference.png"), buf);
}
